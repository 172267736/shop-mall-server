...
var menuItem = Vue.extend({
    ...
    template: [
        ...
        '	<a v-if="item.menuType === 2 && item.parentId === 0" :href="\'#\'+item.url">',
        '		<i v-if="item.icon != null" :class="item.icon"></i>',
        '		<span>{{item.menuName}}</span>',
        '	</a>',

        '	<a v-if="item.menuType === 2 && item.parentId != 0" :href="\'#\'+item.url">',
        '      <i v-if="item.menuName != null" :class="item.icon"></i><i v-else class="fa fa-circle-o"></i> {{item.menuName}}',
        '   </a>',
        '</li>'
    ].join('')
});
...
var vm = new Vue({
    ...
    methods: {
        getMenuList: function (event) {
            $.getJSON(buildUrl("menu/listByUser", null), function (r) {
                if (r.code == "000000") {
                    vm.menuList = r.data;
